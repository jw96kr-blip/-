...
export async function GET(request: NextRequest) {
  ...
  if (searchParams.get('houseNm')) params.set('houseNm', searchParams.get('houseNm')!);
  ...
  try {
    ...
    if (contentType.includes('xml')) {
      ...
      return Response.json(parseXmlResponse<Listing>(xml));
    }

    const json = await res.json();
    return Response.json(parseJsonResponse<Listing>(json));
  } catch (err) {
