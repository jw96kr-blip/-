...
export default function ListingsPage() {
  ...
  const { data, isLoading } = useSWR<ApiResponse<Listing>>(
    `/api/listings?pageNo=${page}&numOfRows=${numOfRows}`,
    fetcher
  );
  ...
  return (
    <>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-8 space-y-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800">분양정보</h1>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          {isLoading ? <LoadingSpinner /> : <ListingsTable items={data?.items ?? []} />}
        </div>
